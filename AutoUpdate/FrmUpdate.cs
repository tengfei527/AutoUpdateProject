...
namespace AutoUpdate
{
    ...
    public class FrmUpdate : System.Windows.Forms.Form
    {
        ...
        {
            this.Close();
            if (!IsMainAppRun()) Process.Start(mainAppExe);
        ...
        private void DownUpdateFile()
        {
                ...
                for (int i = 0; i < this.lvUpdateList.Items.Count; i++)
                {
                    try
                    {
                        ...
                        this.Invoke((MethodInvoker)delegate ()
                        {
                            ...
                            updateFileUrl = updateUrl + lvUpdateList.Items[i].Text.Trim();
                        });
                        long fileLength = 0;
            ...
            {
                MessageBox.Show("更新文件下载失败！" + e.Message.ToString(), "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            finally
            {
