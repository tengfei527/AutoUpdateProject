...
namespace System.Linq.Dynamic
{
    public static class DynamicQueryable
    {
        ...
        public static IQueryable GroupJoin(this IQueryable outer, IQueryable inner,
                string outerKeySelector, string innerKeySelector, string resultSelector,
                params object[] values)
        {
            Type innerElementType = inner.ElementType;
        ...
        public static IQueryable Join(this IQueryable outer, IQueryable inner,
               string outerKeySelector, string innerKeySelector, string resultSelector,
               params object[] values)
        {
            Type innerElementType = inner.ElementType;
            ...
            var innerLambda = DynamicExpression.ParseLambda(new[] { innerParameter },
                outerLambda.Body.Type, innerKeySelector, values);
            var resultLambda = DynamicExpression.ParseLambda(new[] {
                    outerParameter, innerParameter }, null, resultSelector, values);

            return outer.Provider.CreateQuery(Expression.Call(typeof(Queryable),
                "Join", new[] { outer.ElementType, innerElementType,
                    outerLambda.Body.Type, resultLambda.Body.Type },
                outer.Expression, Expression.Constant(inner),
                Expression.Quote(outerLambda), Expression.Quote(innerLambda),
                Expression.Quote(resultLambda)));
        ...
        public static List<object> ToList(this IQueryable source)
        {
            return source.ToListAsync().Result;
        }

        public static async Task<List<object>> ToListAsync(this IQueryable source)
        {
            if (source == null) throw new ArgumentNullException("source");
            IAsyncQueryProvider provider = source.Provider as IAsyncQueryProvider;
            if (provider!=null)
            {
                IAsyncEnumerator<object> e = source.GetType().GetTypeInfo().DeclaredMethods.FirstOrDefault(m => m.ReturnType.GetGenericTypeDefinition() == typeof(IAsyncEnumerator<>)).Invoke(source, null) as IAsyncEnumerator<object>;
        ...
        public static Task<object> FirstAsync(this IQueryable source)
        {
            if (source == null) throw new ArgumentNullException("source");
            IAsyncQueryProvider provider = source.Provider as IAsyncQueryProvider;
            if (provider != null)
    ...
    internal class ClassFactory
    {
        ...
        Type CreateDynamicClass(DynamicProperty[] properties)
        {
            ...
            try
            {
                ...
                tb.SetParent(typeof(DynamicEntity));
                FieldInfo[] fields = GenerateProperties(tb, properties);
                GenerateEquals(tb, fields);
                GenerateGetHashCode(tb, fields);
                Type result = tb.CreateTypeInfo().UnderlyingSystemType;
                classCount++;
                return result;
            }
            finally
        ...
        FieldInfo[] GenerateProperties(TypeBuilder tb, DynamicProperty[] properties) {
            ...
            for (int i = 0; i < properties.Length; i++) {
                ...
                ILGenerator iLGenerator = mbGet.GetILGenerator();
                MethodInfo getMethod = baseType.GetMethod("GetValue").MakeGenericMethod(new Type[] { dp.Type });
                iLGenerator.DeclareLocal(dp.Type);
                iLGenerator.Emit(OpCodes.Nop);
                iLGenerator.Emit(OpCodes.Ldarg_0);
                iLGenerator.Emit(OpCodes.Ldstr, dp.Name);
                iLGenerator.EmitCall(OpCodes.Call, getMethod, null);
                iLGenerator.Emit(OpCodes.Stloc_0);
                iLGenerator.Emit(OpCodes.Ldloc_0);
                iLGenerator.Emit(OpCodes.Ret);
                MethodInfo setMethod = baseType.GetMethod("SetValue").MakeGenericMethod(new Type[] { dp.Type });
                MethodBuilder mbSet = tb.DefineMethod("set_" + dp.Name,
                    MethodAttributes.Public | MethodAttributes.SpecialName | MethodAttributes.HideBySig,
                    null, new Type[] { dp.Type });
                ILGenerator generator2 = mbSet.GetILGenerator();
                generator2.Emit(OpCodes.Nop);
                generator2.Emit(OpCodes.Ldarg_0);
                generator2.Emit(OpCodes.Ldstr, dp.Name);
        ...
        void GenerateEquals(TypeBuilder tb, FieldInfo[] fields) {
            ...
            LocalBuilder other = gen.DeclareLocal(tb.UnderlyingSystemType);
            Label next = gen.DefineLabel();
            gen.Emit(OpCodes.Ldarg_1);
            gen.Emit(OpCodes.Isinst, tb.UnderlyingSystemType);
        ...
        MemberInfo FindPropertyOrField(Type type, string memberName, bool staticAccess) {
            ...
            foreach (Type t in SelfAndBaseTypes(type)) {
                MemberInfo[] members = t.GetTypeInfo().FindMembers(MemberTypes.Property | MemberTypes.Field,
                    flags, new MemberFilter(__Filters.Instance.FilterIgnoreCase), memberName);
        ...
        int FindMethod(Type type, string methodName, bool staticAccess, Expression[] args, out MethodBase method) {
            ...
            foreach (Type t in SelfAndBaseTypes(type)) {
                MemberInfo[] members = t.GetTypeInfo().FindMembers(MemberTypes.Method,
                    flags, new MemberFilter(__Filters.Instance.FilterIgnoreCase), methodName);
        ...
        static object ParseEnum(string name, Type type) {
            if (type.GetTypeInfo().IsEnum) {
                MemberInfo[] memberInfos = type.GetTypeInfo().FindMembers(MemberTypes.Field,
                    ...
                    new MemberFilter(__Filters.Instance.FilterIgnoreCase), name);
